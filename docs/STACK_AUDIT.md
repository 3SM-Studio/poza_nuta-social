...
| Vitest | PASS | 5.0.1; 83 tests cover taxonomy/domain safety, environment/bootstrap handling, login policy, invitation orchestration/reconciliation, token integrity, traffic/sink eligibility, lifecycle state, broad device categorization, dashboard ranges and URL validation. |
...
| CI | PASS (local configuration) / HOSTED EXECUTION NOT VERIFIED | npm 11 and Supabase CLI are pinned, `package-lock.json` is the sole lockfile, verification includes guards/secret scan/Impeccable/lint/types/tests/build, E2E installs Chromium/WebKit, and the database job performs explicit local migration replay, lint, pgTAP, both concurrency suites, scenario-contract checks and isolated Admin browser checks after its own reset. The YAML parsed and all referenced package scripts resolved locally; the updated workflow has not run on GitHub Actions in this continuation. |
