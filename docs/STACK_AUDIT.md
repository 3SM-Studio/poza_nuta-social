...
The SEO/GEO addendum adds factual karaoke and venue pages, route-specific metadata and a linked JSON-LD entity graph, validates active official `sameAs` profiles, keeps the sitemap to durable public pages, and applies `noindex` to preview and non-public responses. Local production build, full public browser coverage and a preview build using the alternate candidate host passed; actual production indexing and domain choice remain release gates. See `docs/seo/SEO_GEO_PRODUCTION_HARDENING.md`.
...
| SEO | PASS (local) / LIVE VERIFICATION PENDING | Five durable public routes with server-rendered HTML, self canonicals and route-specific metadata/OG; truthful Organization/WebSite/WebPage/BreadcrumbList graph and validated active `sameAs`; sitemap, robots, 404/redirect and non-public noindex browser-verified. Final hostname and live indexing remain pending. |
| GEO / AI search | PASS (local) / LIVE VERIFICATION PENDING | Clear Poza Nutą/Trójmiasto entity facts and internal links; `OAI-SearchBot` allowed. GPTBot remains blocked under existing policy. No `llms.txt` or fake event markup. Search Console/Bing setup and citation monitoring remain production work. |
...
| Vitest | PASS | 5.0.1; 96 tests cover taxonomy/domain safety, environment/bootstrap handling, login policy, invitation orchestration/reconciliation, bounded analytics JSON, token integrity, traffic/sink eligibility, lifecycle state, broad device categorization, dashboard ranges, URL validation and SEO entity/canonical-domain rules. |
