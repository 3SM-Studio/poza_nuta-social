...
- [x] `npm run test:db` passes all 197 assertions after an explicit `supabase db reset --local` replays all seven migrations and the seed. The six earlier owner failures were caused by accumulated local E2E fixtures.
...
- [x] Affected local Admin browser cases pass in desktop Chromium after hardening: owner/screenshots, membership, Team/invites, referrals, Sidebar/responsive dialogs and ownership transfer. The public browser suite was previously passing and was not invalidated by these Admin-only edits.
- [x] `npx shadcn@latest info` confirms `base-nova` / Base UI / `cn`.
- [x] CI YAML parses locally; its database job uses explicit local reset/lint flags, resolves all package scripts, and orders migration replay, pgTAP, concurrency, scenario checks and a second local reset before Admin browser tests. Hosted CI execution remains unverified.
...
- [x] PostgreSQL 17 applies all seven migrations and seed from empty state.
